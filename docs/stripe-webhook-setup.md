...
- **Endpoint URL:** `https://api.lordai.net/stripe/webhook` (replace the domain with your **API** host, not the public frontend; there is no extra path beyond `/stripe/webhook`).
...
2. Set the **Endpoint URL** to the full path `https://api.lordai.net/stripe/webhook` (or whatever value you use for `NEXT_PUBLIC_API_BASE`, ending with `/stripe/webhook`).
...
- Success: `https://lordai.net/wallet`
- Cancel: `https://lordai.net/wallet`
