...
- **Endpoint URL:** `https://api.eltx.online/stripe/webhook` (replace the domain with your **API** host, not the public frontend).
...
2. Set the **Endpoint URL** to `https://api.eltx.online/stripe/webhook` (or whatever value you use for `NEXT_PUBLIC_API_BASE`).
