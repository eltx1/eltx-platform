...
const { updateLastScannedBlock } = require('./userScanProgress');
  ...
  async function enqueueUserScan(db, userId, from_block, to_block) {
    ...
    console.log(`[scanJobs] enqueued job=${id} user=${userId} range=${from_block}-${to_block}`);
    return id;
  }
...
async function leaseNextJob(db) {
  ...
  try {
    ...
    await conn.commit();
    console.log(`[scanJobs] leased job=${job.id} user=${job.user_id}`);
    return job;
  } catch (e) {
    await conn.rollback();
    throw e;
  ...
  async function runJob(db, job, scanner) {
    console.log(`[scanJobs] running job=${job.id}`);
    ...
    console.log(`[scanJobs] completed job=${job.id}`);
  }
