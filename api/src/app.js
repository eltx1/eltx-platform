...
const stripeState = createStripeState();
const { applyStripeConfig, getStripeStatusPayload, isStripeEnabled } = stripeState;
const isDemoMode = process.env.DEMO_MODE === 'true' || process.env.NODE_ENV === 'test';
if (!process.env.MASTER_MNEMONIC && isDemoMode) {
  process.env.MASTER_MNEMONIC = 'test test test test test test test test test test test junk';
  console.warn('[api] DEMO_MODE enabled; using fallback MASTER_MNEMONIC.');
}
...
const { provisionUserAddress, getUserBalance } = require('./services/wallet');
...
const GOOGLE_OAUTH_CLIENT_ID = firstNonEmptyEnv(
  process.env.GOOGLE_OAUTH_CLIENT_ID,
  process.env.GOOGLE_CLIENT_ID,
  process.env.GOOGLE_OAUTH_WEB_CLIENT_ID
);
const GOOGLE_OAUTH_CLIENT_SECRET = firstNonEmptyEnv(
...
const PANCAKE_V3_FACTORY_ADDRESS = (
  getRuntimeEnv('PANCAKE_V3_FACTORY') || ''
).toLowerCase();
...
function getSmtpStatus() {
  const emailDefaults = getEmailEnvDefaults();
  const host = normalizeSettingValue(process.env.SMTP_HOST || emailDefaults.host);
  const port = Number(process.env.SMTP_PORT || emailDefaults.port) || 587;
  const user = normalizeSettingValue(process.env.SMTP_USER || emailDefaults.user);
  const pass = normalizeSettingValue(process.env.SMTP_PASS || emailDefaults.pass);
  const from = normalizeSettingValue(process.env.SMTP_FROM || emailDefaults.fromAddress) || user || null;
  const missing = [];
...
function resolveConvertAssetAddress(assetSymbol, pairAddress = null) {
  ...
  if (upper === 'BUSD') return BSC_BUSD_ADDRESS;
...
function buildConvertRouteCandidates(pair, side) {
  ...
  const bridges = [resolveConvertAssetAddress('WBNB'), resolveConvertAssetAddress('USDC'), resolveConvertAssetAddress('BUSD')]
    .filter(Boolean)
    .map((item) => String(item).toLowerCase())
    .filter((item, idx, arr) => arr.indexOf(item) === idx);
  const start = side === 'buy' ? quoteAddress : baseAddress;
  const end = side === 'buy' ? baseAddress : quoteAddress;
  const direct = [start, end];
  const routes = [direct];
  for (const bridge of bridges) {
    if (bridge === start || bridge === end) continue;
    routes.push([start, bridge, end]);
  }
...
function routeToSymbols(path) {
  ...
  return path.map((address) => {
    const found = Object.entries(byAddress).find(([, meta]) => String(meta?.address || '').toLowerCase() === String(address).toLowerCase());
    return found?.[0] || `${address.slice(0, 6)}...${address.slice(-4)}`;
  });
}

function logConvertQuoteDiagnostics(context) {
...
async function quoteConvertFromPancakeV2(pair, side, amountWei, provider) {
  ...
  let best = null;
  for (const path of buildConvertRouteCandidates(pair, side)) {
    const routeLabel = routeToSymbols(path).join(' -> ');
    try {
      if (side === 'buy') {
        const amounts = await router.getAmountsIn(amountWei, path);
        const amountIn = bigIntFromValue(Array.isArray(amounts) ? amounts[0] : 0);
        const amountOut = bigIntFromValue(Array.isArray(amounts) ? amounts[amounts.length - 1] : 0);
        if (amountIn > 0n && amountOut > 0n && (!best || amountIn < best.quoteWithoutFeeWei)) {
          best = { quoteWithoutFeeWei: amountIn, baseAmountWei: amountOut, direction: 'quote_to_base', path };
        }
        attemptedPaths.push({ route: routeLabel, amountIn: amountIn.toString(), amountOut: amountOut.toString(), ok: amountIn > 0n && amountOut > 0n });
      } else {
        const amounts = await router.getAmountsOut(amountWei, path);
        const amountIn = bigIntFromValue(Array.isArray(amounts) ? amounts[0] : 0);
        const amountOut = bigIntFromValue(Array.isArray(amounts) ? amounts[amounts.length - 1] : 0);
        if (amountOut > 0n && amountIn > 0n && (!best || amountOut > best.quoteWithoutFeeWei)) {
          best = { quoteWithoutFeeWei: amountOut, baseAmountWei: amountIn, direction: 'base_to_quote', path };
        }
        attemptedPaths.push({ route: routeLabel, amountIn: amountIn.toString(), amountOut: amountOut.toString(), ok: amountOut > 0n && amountIn > 0n });
      }
    } catch (err) {
  ...
  if (!best) {
    const error = Object.assign(new Error(`No PancakeSwap V2 route found for ${pair.symbol}`), { code: 'LIVE_QUOTE_UNAVAILABLE', attemptedPaths });
    throw error;
  }
...
async function quoteConvertFromPancakeV3(pair, side, amountWei, provider) {
  if (!PANCAKE_V3_FACTORY_ADDRESS || !/^0x[a-f0-9]{40}$/i.test(PANCAKE_V3_FACTORY_ADDRESS)) {
    throw Object.assign(new Error('PANCAKE_V3_FACTORY is missing/invalid'), { code: 'INVALID_ENV_PANCAKE_V3_FACTORY' });
  }
  ...
  let best = null;
  const attemptedPaths = [];
  const factory = new ethers.Contract(PANCAKE_V3_FACTORY_ADDRESS, PANCAKE_V3_FACTORY_ABI, provider);
    ...
    for (let combo = 0; combo < maxCombos; combo += 1) {
      ...
      try {
        ...
        for (let i = 0; i < hops; i += 1) {
          const pool = String(await factory.getPool(tokens[i], tokens[i + 1], Number(fees[i]))).toLowerCase();
          if (!/^0x[a-f0-9]{40}$/.test(pool) || pool === '0x0000000000000000000000000000000000000000') {
            poolIssue = { hop: i, status: 'poolMissing', tokenIn: tokens[i], tokenOut: tokens[i + 1], fee: Number(fees[i]) };
            break;
          }
          try {
            const poolCtr = new ethers.Contract(pool, PANCAKE_V3_POOL_ABI, provider);
            const liq = bigIntFromValue(await poolCtr.liquidity());
            if (liq <= 0n) {
              poolIssue = { hop: i, status: 'liquidityZero', tokenIn: tokens[i], tokenOut: tokens[i + 1], fee: Number(fees[i]), pool };
              break;
            }
          } catch {
            // ignore optional liquidity check errors
          }
        }
        if (poolIssue) {
          attemptedPaths.push({ route: routeToSymbols(tokens).join(' -> '), fees, ok: false, ...poolIssue });
          continue;
        }
        ...
        attemptedPaths.push({ route: routeToSymbols(tokens).join(' -> '), fees, amountOut: amountOut.toString(), ok: amountOut > 0n, status: amountOut > 0n ? 'quoteOk' : 'quoteFailed' });
        if (amountOut > 0n && (!best || amountOut > best.amountOutWei)) {
          best = { tokens, fees, pathBytes, amountOutWei: amountOut };
        }
      } catch (err) {
        attemptedPaths.push({ route: routeToSymbols(tokens).join(' -> '), fees, ok: false, status: 'quoteFailed', error: String(err?.message || err || 'v3_quote_failed').slice(0, 180) });
      }
    }
  }
...
async function executeConvertOnPancake(pair, side, executionPlan, runtime) {
  if (runtime.mode !== 'live') {
    ...
    const mockOutWei = side === 'buy' ? executionPlan.expectedBaseOutWei : executionPlan.expectedQuoteOutWei;
    return {
      txHash: `mock-${Date.now()}`,
      receivedQuoteWei: executionPlan.expectedQuoteOutWei,
      receivedBaseWei: executionPlan.expectedBaseOutWei,
      expectedOutWei: mockOutWei,
      minOutWei: mockOutWei,
      swapInputWei: executionPlan.swapInputWei,
      feeTier: runtime.quoteFeeTier || null,
      mode: 'mock',
    };
  }
...
app.post('/stripe/webhook', async (req, res) => {
  if (!stripeState.stripe || !stripeState.stripeWebhookSecret) {
  ...
  let event;
  try {
    event = stripeState.stripe.webhooks.constructEvent(req.body, signature, stripeState.stripeWebhookSecret);
  } catch (err) {
...
app.post('/fiat/stripe/session', walletLimiter, async (req, res, next) => {
  ...
  try {
    ...
    const session = await stripeState.stripe.checkout.sessions.create({
      mode: 'payment',
      payment_method_types: ['card'],
      client_reference_id: String(purchaseId),
      customer_email: userRow?.email || undefined,
      success_url: stripeState.stripeSuccessUrl,
      cancel_url: stripeState.stripeCancelUrl,
...
app.get('/convert/health', walletLimiter, async (req, res, next) => {
  try {
    ...
    let routeCheck = { routeFound: false, routeSymbols: [], reason: null, attemptedRoutes: [] };
    ...
    if (provider && runtime.envReady) {
      try {
        ...
        if (bigIntFromValue(probeAmount) > 0n) {
          ...
          routeCheck = { routeFound: liquidityRouteFound, routeSymbols: quoteInfo.routeSymbols || [], reason: liquidityRouteFound ? null : 'PAIR_ROUTE_UNAVAILABLE', attemptedRoutes: quoteInfo.attemptedPaths || [] };
        }
      } catch (err) {
    ...
    res.json({
      ...
      attemptedRoutes: routeCheck.attemptedRoutes || [],
      quoteProvider,
...
app.post('/convert/execute', walletLimiter, async (req, res, next) => {
  ...
  try {
    ...
    let grossUsdtInputWei = 0n;
    let netUsdtToSwapWei = 0n;
    if (isBuyInput) {
      const amountUsdtWeiStr = decimalToWeiString(payload.amountUsdt, usdtDecimals);
      if (!amountUsdtWeiStr) return next({ status: 400, code: 'INVALID_AMOUNT', message: 'Invalid amountUsdt' });
      grossUsdtInputWei = bigIntFromValue(amountUsdtWeiStr);
      if (grossUsdtInputWei <= 0n) return next({ status: 400, code: 'INVALID_AMOUNT', message: 'Invalid amountUsdt' });
      amountWei = grossUsdtInputWei;
    } else {
    ...
    if (runtime.mode === 'live') {
      ...
      const provider = new ethers.JsonRpcProvider(runtime.rpcUrl);
      if (payload.side === 'buy') {
        feeWei = (grossUsdtInputWei * BigInt(settings.convert_fee_bps || 0)) / (10000n + BigInt(settings.convert_fee_bps || 0));
        netUsdtToSwapWei = grossUsdtInputWei - feeWei;
      }
      quoteInfo = payload.side === 'buy'
        ? await quoteExactQuoteToBase(pair, netUsdtToSwapWei, provider)
        : await quoteExactBaseToQuote(pair, amountWei, provider);
      ...
      if (payload.side === 'buy') {
        feeWei = (grossUsdtInputWei * BigInt(settings.convert_fee_bps || 0)) / (10000n + BigInt(settings.convert_fee_bps || 0));
        netUsdtToSwapWei = grossUsdtInputWei - feeWei;
        quoteInfo = { quoteWithoutFeeWei: netUsdtToSwapWei, baseAmountWei: quoteConvertMock(pair, netUsdtToSwapWei), fee: null };
      } else {
    ...
    debitWei = amountWei;
    feeWei = payload.side === 'buy' ? feeWei : (quoteInfo.quoteWithoutFeeWei * BigInt(settings.convert_fee_bps || 0)) / 10000n;
    swapInputWei = payload.side === 'buy' ? netUsdtToSwapWei : amountWei;
    ...
    res.json({
      ...
      fee: { asset: 'USDT', amount: trimDecimal(formatUnitsStr(feeWei.toString(), usdtDecimals)) },
      net_swap_input: { asset: payload.side === 'buy' ? 'USDT' : pair.base_asset, amount: trimDecimal(formatUnitsStr(swapInputWei.toString(), payload.side === 'buy' ? usdtDecimals : baseDecimals)) },
...
app.post('/admin/convert/pairs/:id/probe-route', async (req, res, next) => {
  ...
  try {
    ...
    const runtime = await buildConvertRuntime();
    const provider = new ethers.JsonRpcProvider(runtime.rpcUrl);
    const buyIn = bigIntFromValue(decimalToWeiString(String(body.amountUsdt || '1'), resolveConvertAssetDecimals(pair.quote_asset)) || '0');
    const sellIn = bigIntFromValue(decimalToWeiString(String(body.baseAmount || '0.0003'), resolveConvertAssetDecimals(pair.base_asset, pair.token_decimals)) || '0');
    let buy = { executable: false };
    let sell = { executable: false };
    try { const q = await quoteConvertFromPancakeV3({ ...pair, route_mode: mode }, 'buy', buyIn, provider); buy = { executable: true, bestRoute: { pathSymbols: q.routeSymbols, pathTokens: q.path, feeTiers: q.feeTiers, amountInWei: buyIn.toString(), amountOutWei: q.baseAmountWei.toString(), pathBytes: q.pathBytes }, attemptedRoutes: q.attemptedPaths }; } catch (e) { buy = { executable: false, error: String(e?.message || e), attemptedRoutes: e?.attemptedPaths || [] }; }
    try { const q = await quoteConvertFromPancakeV3({ ...pair, route_mode: mode }, 'sell', sellIn, provider); sell = { executable: true, bestRoute: { pathSymbols: q.routeSymbols, pathTokens: q.path, feeTiers: q.feeTiers, amountInWei: sellIn.toString(), amountOutWei: q.quoteWithoutFeeWei.toString(), pathBytes: q.pathBytes }, attemptedRoutes: q.attemptedPaths }; } catch (e) { sell = { executable: false, error: String(e?.message || e), attemptedRoutes: e?.attemptedPaths || [] }; }
    if (save) {
      await pool.query(
        `UPDATE convert_pairs SET
          last_working_buy_route_json=?,
          last_working_sell_route_json=?,
          last_route_probe_status=?,
          last_route_probe_error=?,
          last_route_probe_at=NOW(),
          updated_at=NOW()
         WHERE id=?`,
        [
          buy?.bestRoute ? JSON.stringify(buy.bestRoute) : null,
          sell?.bestRoute ? JSON.stringify(sell.bestRoute) : null,
          buy.executable || sell.executable ? 'ok' : 'failed',
          buy.executable || sell.executable ? null : String(buy.error || sell.error || 'route_probe_failed').slice(0, 500),
          pairId,
        ]
      );
    }
    res.json({ ok: true, pair: pair.symbol, buy, sell, save });
  } catch (err) {
    next(err);
  }
});
...
const { getLatestBlockNumber } = require('./services/bscRpc');
const { enqueueUserScan, getJobStatus } = require('./services/scanJobs');
const { getLastScannedBlock } = require('./services/userScanProgress');
