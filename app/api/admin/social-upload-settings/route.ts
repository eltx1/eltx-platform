...
import { normalizeSocialUploadSettings } from '../../../lib/social-upload-settings';
import { readSocialUploadSettings, writeSocialUploadSettings } from '../../../lib/social-upload-settings.server';
...
async function requireAdminAuthorization() {
  ...
  try {
    ...
    console.error('social upload settings admin auth error', error);
...
export async function PUT(request: NextRequest) {
  ...
  const settings = normalizeSocialUploadSettings(body?.settings || body);
  const savedSettings = await writeSocialUploadSettings(settings);
