...
import { readAdsFilesSettings, writeAdsFilesSettings } from '../../../lib/ads-files.server';
...
async function requireAdminAuthorization() {
  ...
  try {
    ...
    console.error('ads files admin auth error', error);
...
export async function PUT(request: NextRequest) {
  ...
  const settings = await writeAdsFilesSettings(body?.settings || body);
  return NextResponse.json({ settings });
}
